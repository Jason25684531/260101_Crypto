...
│   ├── manual/                  # 手動測試腳本 (新增 2026-01-31)
│   │   ├── README.md            # 手動測試說明文件
│   │   ├── test_bot.py          # Bot 功能驗證
│   │   ├── test_jobs_manual.py  # Data Jobs 驗證
│   │   ├── test_scheduler_manual.py  # Scheduler 驗證
│   │   ├── test_paper_trading_manual.py  # Paper Trading 驗證
│   │   ├── test_paper_simple.py # 簡化版 Paper Trading 測試
│   │   ├── test_ml_predictor.py # ML 預測器測試
│   │   └── test_kill_switch.py  # Kill Switch 測試
│   ├── integration/             # (保留) 整合測試
...
├── logs/                         # 日誌目錄（已忽略）
│
├── UpdateList.md                 # 代碼重構與清理更新記錄 (新增 2026-01-31)
├── .env                          # 環境變數（已忽略）
├── .env.example                  # 環境變數範本
├── .gitignore                    # Git 忽略規則
...
1. **測試檔案重組**
   - 移動 7 個散落的測試檔案到 `tests/manual/`
   - 創建 `tests/manual/README.md` 說明文件
   - 根目錄結構更清晰

2. **Fetcher 架構優化**
   - 抽取共用的數據庫操作邏輯 `_save_ohlcv_to_db()`
   - 減少 MarketFetcher 和 BinanceFetcher 的重複代碼
   - 保持異步/同步版本的獨立性
   - 減少約 50 行重複代碼

3. **新增文件**
   - ✅ `UpdateList.md` - 詳細的重構記錄文件
...
- MySQL +（2026-01-31 重構完成）
- ✅ 無未使用的檔案
- ✅ 標準 Pytest 測試架構
- ✅ 清晰的模組分離（MVC 模式）
- ✅ 測試檔案分類管理（unit/ 與 manual/ 分離
...
**最近更新 (2026-01-31)**:
- ✅ 測試檔案結構重組（移動 7 個檔案到 tests/manual/）
- ✅ Fetcher 架構重構（抽取共用數據庫邏輯）
- ✅ 新增 UpdateList.md 詳細記錄變更
- ✅ 減少約 50 行重複代碼
